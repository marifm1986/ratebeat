              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {users.map((user) => (
                  <tr key={user.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${user.role === 'admin' ? 'bg-purple-100 text-purple-800' : user.role === 'editor' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}
                      >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-wrap gap-1">
                        {user.accessAreas.map((area) => (
                          <span
                            key={area}
                            className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800"
                          >
                            {ACCESS_AREAS.find((a) => a.id === area)?.name ||
                              area}
                          </span>
                        ))}
                  ...
                  {ACCESS_AREAS.map((area) => (
                    <div key={area.id} className="flex items-center">
                      <input
                        type="checkbox"
                        id={`area-${area.id}`}
                        checked={formData.accessAreas.includes(area.id)}
                        onChange={() => handleAccessAreaChange(area.id)}
                        disabled={
                          formData.role === 'admin' && area.id === 'dashboard'
                        }
                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      />
                      <label
                        htmlFor={`area-${area.id}`}
                        className={`ml-2 text-sm ${formData.role === 'admin' && area.id === 'dashboard' ? 'text-gray-500' : 'text-gray-700'}`}
                      >
                        {area.name}
                        {area.id === 'dashboard' &&
                          formData.role === 'admin' &&
